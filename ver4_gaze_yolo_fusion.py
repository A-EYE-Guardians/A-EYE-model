...
"""
ver4_gaze_yolo_fusion.py

- Eye×World + Video-Depth-Anything(in-proc)
- 변경 사항(Ver4):
  1) --show_eye 옵션 추가: 아이캠 프리뷰 창(Eye (FaceMesh)) 표시
  2) 아이캠 처리 조건 버그 수정: 루프 내에서 face_mesh is not None 기준으로 동작
  3) 실행 로그 개선
            ...
            if (face_mesh is not None) and not (args.disable_gaze or args.force_center):
                ...
                if run_mesh and okE:
                    ...
                    frame_eye_vis, gaze = single_cam_dual_iris_step(
                        ...
                        flip=False, clahe=args.clahe_eye, draw=draw_eye
                    )
                    if draw_eye:
                        cv2.imshow("Eye (FaceMesh)", frame_eye_vis)
