...
"""
ver4_inproc_gaze_depth_fusion_webcam.py
- VDA(Depth-Anything v2-stream) in-proc, 세그먼트/서브프로세스 제거
- FaceMesh 실제 실행률 표시(eye_run Hz) + stride 제어
- EMA 튜닝 노출
- 레이×깊이 교차 유효성 필터(res_max, r_min)
- intrinsics K를 월드 프레임 실제 해상도에 맞게 자동 스케일(Kref_w/h 기준)
- 거부 프레임도 회색 마커로 표시(--draw_rejected)
- 얼굴 미검출 시 NO FACE 표기, 눈 화살표(옵션 draw=True일 때)
- --auto_flip_dir: d와 -d 둘 다 시도해서 residual이 작은 쪽 선택(부호 모호성 제거)
- [NEW] 시선 추정 실패/거부 시 월드 화면 정중앙 십자가 fallback 표시(기본 on, --no_fallback_center로 끌 수 있음)
"""
...
def main():
    ...
    # intrinsics가 유효한 기준 해상도(없으면 --width_world/--height_world로 가정)
    ...
    # [NEW] 중앙 십자가 fallback on/off
    ap.add_argument("--no_fallback_center", action="store_true",
                    help="시선 실패/거부 시 중앙 십자가 표시를 끄기")
    ...
    print(f"[Args] Stride={args.eye_stride}  EMA={args.ema}  Filters(res_max={args.res_max}, r_min={args.r_min})", flush=True)
    ...
    def _warmup(get_fn, name, timeout=3.0):
        ...
        return ok, shape
    okE, shapeE = _warmup(read_eye,"eye_cam")
    ...
    if not (okE and okW):
        ...
        try: rel_eye(); rel_world(); face_mesh.close(); cv2.destroyAllWindows()
        except: pass
        return
    ...
    try:
        while True:
            ...
            if last_gaze is not None and depth_m is not None:
                ...
                if P_hit is not None and u is not None:
                    ...
                    if passed:
                        ...
                        if not args.no_gui:
                            cv2.drawMarker(frame_world, (u,v), (0,0,255), cv2.MARKER_CROSS, 16, 2)  # valid: red
                    else:
                        why = []
                        ...
                        if not args.no_gui and args.draw_rejected:
                            cv2.drawMarker(frame_world, (u,v), (180,180,180), cv2.MARKER_TILTED_CROSS, 14, 1)  # rejected: gray
                else:
                    hit_txt = "fix: not found"

            # [NEW] Fallback center crosshair (기본 on)
            if not valid_fix and not args.no_gui and not args.no_fallback_center:
                center_uv = (Wc//2, Hc//2)
                # 시각적으로 구분되도록 노란 십자가
                cv2.drawMarker(frame_world, center_uv, (0,255,255), cv2.MARKER_CROSS, 16, 2)
                if hit_uv is None:
                    hit_txt = "fix: fallback=center (no gaze/depth)"
                else:
                    hit_txt += " | fallback=center"
            ...
            if now - t_last >= 1.0:
                ...
                mesh_hz = mesh_runs / span if span > 0 else 0.0  # 확정값 갱신
                print(f"[FPS] eye_run={mesh_hz:4.1f}/s world_loop={world_fps:4.1f} | {hit_txt}", flush=True)
                world_frames = 0; mesh_runs = 0; t_last = now

            # GUI
            if not args.no_gui:
                if args.show_fps:
                    span_live = max(time.time() - t_last, 1e-3)
                    mesh_hz_disp = (mesh_runs / span_live) if span_live < 1.0 else mesh_hz
                    cv2.putText(frame_eye_vis, f"FaceMesh Hz: {mesh_hz_disp:4.1f}", (12,28),
