...
"""
ver6_inproc_gaze_depth_fusion_webcam_yolo_docker.py
...
- [YOLOE(도커) 연동] 'R' 키를 누를 때만 현재 프레임(오버레이 그리기 전의 깨끗한 프레임)과
  fix 좌표(u_fix, v_fix)를 FastAPI 서버(/infer)에 전송 → 서버가 ROI 저장 및 bbox 반환
  ※ 클라이언트 쪽은 단순 HTTP 업로드만 수행. 이벤트 저장/ROI 저장은 서버 쪽에서 처리.

Hotkeys:
  R / r : 현재 프레임에서 fix(빨강/노랑 십자) 좌표 기준으로 YOLOE 1회 실행(도커 HTTP 호출)
  Q / ESC : 종료
"""

import os, sys, json, time, math, argparse, contextlib, importlib, importlib.util, types, io
...
def _open_pyav_or_fallback(src, *, av_backend, pixel_format, width, height, fps,
    ...
    """카메라/RTSP 열기 (PyAV 우선, 실패시 OpenCV)."""
    use_pyav = False
    ...
    def infer_one(self, bgr, uv: Tuple[int,int], imgsz: Optional[int]=None,
        ...
        try:
            ...
            if r.status_code != 200:
                ...
                return None
            resp = r.json()
            # 기대 필드: bbox {x1,y1,x2,y2}, label, conf, image_url
            bbox = resp.get("bbox")
...
# ===== Main =====
def main():
    ap = argparse.ArgumentParser(description="Eye×World (webcam/RTSP) + VDA (in-proc) + YOLOE Docker one-shot")
    ...
    try:
        while True:
            ...
            # ★ 오버레이 그리기 전에 '깨끗한 프레임' 보관 (YOLO 전송용)
            ...
            if not args.no_gui:
                ...
                if k in (27, ord('q')):
                    ...
                    # --- YOLO one-shot: '오버레이 전' 깨끗한 프레임을 사용 ---
                    if yolo_client is None:
                        ...
                        if out is None:
                            ...
                            # 화면에는 표시용 오버레이만 그려줌(저장은 서버에서 수행)
