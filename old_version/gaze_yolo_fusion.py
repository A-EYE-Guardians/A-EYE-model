...
# -*- coding: utf-8 -*-
"""
ver5_inproc_gaze_depth_fusion_webcam_yolo.py
- 기존 ver5_inproc_gaze_depth_fusion_webcam.py 에 'R' 키 누를 때만 1회 YOLO(E) 실행하여
  현재 fix 위치(또는 중앙) 주변 객체를 선택 → ROI 크롭 저장 + 이벤트 로그 기록 기능 추가
- 메모리 절약: YOLO는 'R' 누를 때만 Lazy Load → 1회 추론 → 즉시 언로드(옵션)

Hotkeys:
  R / r : 현재 프레임에서 fix(빨강/노랑 십자) 좌표 기준으로 YOLO 1회 실행 → 가장 가까운 박스 선택 → ROI 저장
  Q / ESC : 종료

추가 인자:
  --enable_yolo                  YOLO 기능 켜기(미지정시 비활성)
  --yolo_weights PATH            가중치(기본: yoloe-11s-seg-pf.pt)
  --yolo_device DEV              'cpu' 또는 CUDA 인덱스('0' 등)
  --yolo_conf F                  0~1, 기본 0.25
  --yolo_iou F                   0~1, 기본 0.50
  --yolo_imgsz N                 입력 크기, 기본 640
  --yolo_unload_after_run        1회 실행 뒤 즉시 언로드하여 VRAM 해제(기본: 꺼짐)
  --yolo_outdir PATH             ROI/이벤트 저장 루트 (기본: ./uploads)

주의:
- YOLOE 가 ultralytics 버전에 없을 수도 있어 YOLO로 폴백합니다.
- 외부 인터넷 미사용. 경로만 로컬로 기록합니다.
"""
...
# ===== YOLO One-Shot Runner (내장) =====
class OneShotYolo:
    def __init__(self, weights: str, device: Optional[str], conf: float, iou: float, imgsz: int,
                 out_root: str):
        # ultralytics import (YOLOE → fallback YOLO)
        try:
            from ultralytics import YOLOE as _UL_MODEL_CLS
        except Exception:
            from ultralytics import YOLO as _UL_MODEL_CLS  # type: ignore
        self._Model = _UL_MODEL_CLS
        self.model = None
        self.names = {}
        ...
        self.roi_dir = os.path.join(self.out_root, 'roi')
        os.makedirs(self.roi_dir, exist_ok=True)
        self.event_log = os.path.join(self.out_root, 'events.jsonl')

    def _load(self):
        if self.model is None:
            self.model = self._Model(self.weights)
            try:
    ...
    @staticmethod
    def _pick_by_point(xyxy: np.ndarray, click_xy: Tuple[int,int]) -> int:
    ...
    def _safe_crop_jpeg(self, frame_bgr, xyxy, quality: int=90) -> bytes:
        x1,y1,x2,y2 = map(int, xyxy)
        h,w = frame_bgr.shape[:2]
        x1 = max(0, min(x1, w-1)); x2 = max(0, min(x2, w-1))
        y1 = max(0, min(y1, h-1)); y2 = max(0, min(y2, h-1))
        if x2 <= x1 or y2 <= y1: raise ValueError(f"Invalid ROI: {xyxy}")
        roi = frame_bgr[y1:y2, x1:x2]
        ok, buf = cv2.imencode('.jpg', roi, [int(cv2.IMWRITE_JPEG_QUALITY), int(quality)])
        if not ok: raise RuntimeError('Failed to encode ROI JPEG')
        return buf.tobytes()

    def _save_roi_and_get_url(self, frame_bgr, xyxy, label: str) -> str:
        ts_ms = int(time.time()*1000)
        fname = f"{ts_ms}_{label}.jpg"
        jpeg = self._safe_crop_jpeg(frame_bgr, xyxy)
        with open(os.path.join(self.roi_dir, fname), 'wb') as f:
            f.write(jpeg)
        # 단순 로컬 경로 반환(서버에서 /media/roi 로 매핑할 수 있음)
        return os.path.join('media', 'roi', fname)

    def _append_event(self, payload: dict):
        with open(self.event_log, 'a', encoding='utf-8') as f:
            f.write(json.dumps(payload, ensure_ascii=False) + '\n')

    def _build_payload(self, frame_bgr, xyxy, label: str, conf: float, url: str) -> dict:
        ts_ms = int(time.time()*1000)
        h,w = frame_bgr.shape[:2]
        x1,y1,x2,y2 = map(int, xyxy)
        return {
    ...
    def run_once(self, frame_bgr, click_xy: Tuple[int,int]):
        self._load()
        if frame_bgr is None: return None
        h,w = frame_bgr.shape[:2]
        results = self.model.predict(
            source=frame_bgr, imgsz=self.imgsz, conf=self.conf, iou=self.iou,
            agnostic_nms=False, max_det=50, device=self.device, stream=False, verbose=False
        )
        res = results[0]
        boxes = getattr(res, 'boxes', None)
        names = getattr(res, 'names', None) or self.names
        if names and not self.names: self.names = names
        ...
        idx = self._pick_by_point(xyxy, click_xy)
        if idx < 0: return None
        x1,y1,x2,y2 = map(float, xyxy[idx])
        label = self.names.get(int(cls[idx]), str(int(cls[idx])))
        url = self._save_roi_and_get_url(frame_bgr, (x1,y1,x2,y2), label)
        payload = self._build_payload(frame_bgr, (x1,y1,x2,y2), label, float(conf[idx]), url)
...
def main():
    ap = argparse.ArgumentParser(description="Eye×World (webcam/RTSP) + VDA (in-proc) + One‑Shot YOLO")
    ...
    # === YOLO 옵션 ===
    ap.add_argument("--enable_yolo", action="store_true", help="R 키로 YOLO One‑Shot ROI 저장 기능 사용")
    ap.add_argument("--yolo_weights", type=str, default="yoloe-11s-seg-pf.pt")
    ap.add_argument("--yolo_device", type=str, default=None, help="'cpu' 또는 CUDA 인덱스 문자열 예:'0'")
    ...
    ap.add_argument("--yolo_unload_after_run", action="store_true")
    ap.add_argument("--yolo_outdir", type=str, default="./uploads")
    ...
    if args.enable_yolo:  print("[Args] YOLO = enabled (press 'R' to run once)", flush=True)
    ...
    # YOLO 준비(지연 로드)
    yolo_runner: Optional[OneShotYolo] = None

    print("[Main] Entering main loop. Press 'q' or ESC to exit. Press 'R' for YOLO one‑shot.", flush=True)
    try:
        while True:
            ...
            if face_mesh is not None and not (args.disable_gaze or args.force_center):
                ...
                last_gaze = None  # disable/force일 때는 항상 None
            ...
            if (not args.force_center) and (last_gaze is not None) and (depth_m is not None):
                ...
                if P_hit is not None and u is not None:
                    ...
                    if passed:
                        hit_txt = f"fix: u={u_fix}, v={v_fix} | Z={Z_d:.2f}m | R={Rm:.2f}m | res={res:.03f}m | dir={'+' if sign>0 else '-'}"
                        if not args.no_gui:
            ...
            if not args.no_gui:
                ...
                if args.enable_yolo:
                    cv2.putText(frame_world, "Press 'R' for YOLO one-shot ROI", (10,76), cv2.FONT_HERSHEY_SIMPLEX, 0.62, (0,200,255), 2, cv2.LINE_AA)
                ...
                if k in (27, ord('q')):
                    ...
                    # --- YOLO one-shot: 현재 프레임/좌표 사용 ---
                    if yolo_runner is None:
                        yolo_runner = OneShotYolo(
                            weights=args.yolo_weights,
                            device=args.yolo_device,
                            conf=args.yolo_conf,
                            iou=args.yolo_iou,
                            imgsz=args.yolo_imgsz,
                            out_root=args.yolo_outdir,
                        )
                        print("[YOLO] model loaded lazily.", flush=True)
                    # 안전하게 복사본 사용(표시용 프레임 변형이 들어가기 때문)
                    bgr_for_yolo = frame_world.copy()
                    out = yolo_runner.run_once(bgr_for_yolo, (u_fix, v_fix))
                    if out is None:
                        print(f"[YOLO] no box found near ({u_fix},{v_fix})", flush=True)
                    else:
                        (x1,y1,x2,y2) = out['bbox']
                        label, conf = out['label'], out['conf']
                        print(f"[YOLO] Selected: {label}({conf:.2f}) bbox=({x1},{y1},{x2},{y2}) url={out['image_url']}", flush=True)
                        if not args.no_gui:
                            cv2.rectangle(frame_world, (x1,y1), (x2,y2), (0,128,255), 2)
                            cv2.putText(frame_world, f"{label} {conf:.2f}", (x1, max(20,y1-6)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,128,255), 2, cv2.LINE_AA)
                            cv2.imshow("World (VDA in-proc + Fixation)", frame_world)
                            cv2.waitKey(1)
                    if args.yolo_unload_after_run and yolo_runner is not None:
                        yolo_runner.unload(); yolo_runner = None
                        print("[YOLO] unloaded to free VRAM.", flush=True)
        ...
        try:
            if 'yolo_runner' in locals() and yolo_runner is not None:
                yolo_runner.unload()
        except Exception: pass
