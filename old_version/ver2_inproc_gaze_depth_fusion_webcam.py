...
"""
inproc_gaze_depth_fusion_webcam.py  (v2: stride/EMA/validity filter 강화)
- VDA를 같은 프로세스에서 1회 로드 → 매 프레임 추론 (세그먼트/서브프로세스 제거)
- 원인 (1) FaceMesh 실제 업데이트율 가시화 & stride 제어
- 원인 (2) EMA 반응성 쉽게 튜닝 (alpha 노출)
- 원인 (4) 레이×깊이 교차 결과에 유효성 필터(res_max, r_min) 추가
"""

import os, sys, json, time, math, argparse, contextlib, importlib, importlib.util, types
from typing import Optional

# ===== Thread throttling (필수: Windows/BLAS/OpenCV 과다스레드 억제) =====
...
# ===== Utils =====
class EMA:
    def __init__(self, alpha: float=0.5):  # 기본값을 더 민첩하게(0.5) 설정
...
class VDAInproc:
    ...
    def __init__(self, vda_dir: str, encoder: str='vits', metric: bool=True,
        ...
        # import VDA with package context
...
def single_cam_dual_iris_step(frame_bgr, face_mesh, ema_L, ema_R,
    ...
    if not res.multi_face_landmarks: return frame_bgr, out
...
def main():
    ...
    def _warmup(get_fn, name, timeout=3.0):
        ...
        return ok
    if not (_warmup(read_eye,"eye_cam") and _warmup(read_world,"world_cam")):
    ...
    eye_frame_idx = 0
    last_gaze = None
    mesh_runs = 0  # FaceMesh 실제 실행 횟수 카운트
    mesh_hz = 0.0
        ...
        while True:
            ...
            if run_mesh:
                ...
                if gaze["oL"] is not None and gaze["oR"] is not None:
                    ...
                    s = np.array([1.0, float(W_e)/float(H_e), 1.0], np.float32)  # 비등방 보정
            ...
            if last_gaze is not None and depth_m is not None:
                ...
                P_hit,u,v,Z_d,t_hit,res = ray_depth_intersection(
                    O_w,d_w,depth_m,args.fx_w,args.fy_w,args.cx_w,args.cy_w,
                ...
                if P_hit is not None and u is not None:
                    ...
                    if (res <= args.res_max) and (Rm >= args.r_min):
            ...
            # 1초마다 FaceMesh 실제 실행률/루프 FPS 로그
            now = time.time()
            if now - t_last >= 1.0:
                span = (now - t_last)
                world_fps = world_frames / span if span > 0 else 0.0
                mesh_hz = mesh_runs / span   # ← ★ 1초 집계로 확정값 갱신
            ...
            if not args.no_gui:
                if args.show_fps:
                    # 1초가 지나기 전에는 즉시 추정치(현재 윈도우 값), 이후에는 마지막 확정값(mesh_hz) 표시
