...
"""
ver3_inproc_gaze_depth_fusion_webcam.py
- VDA(Depth-Anything v2-stream) in-proc, 세그먼트/서브프로세스 제거
- FaceMesh 실제 실행률 표시(eye_run Hz) + stride 제어
- EMA 튜닝 노출
- 레이×깊이 교차 유효성 필터(res_max, r_min)
- [NEW] intrinsics K를 월드 프레임 실제 해상도에 맞게 자동 스케일
- [NEW] 거부 프레임도 회색 마커로 그리는 옵션(--draw_rejected)
- [NEW] 얼굴 미검출 시 NO FACE 표기, 눈 화살표(옵션 draw=True일 때)
"""
...
class VDAInproc:
    ...
    def __init__(self, vda_dir: str, encoder: str='vits', metric: bool=True,
        ...
        # import module with package context
...
def main():
    ...
    # [NEW] intrinsics가 유효한 기준 해상도(없으면 --width_world/--height_world로 가정)
    ...
    try:
        while True:
            ...
            if last_gaze is not None and depth_m is not None:
                ...
                if P_hit is not None and u is not None:
                    ...
                    if passed:
                        ...
                        hit_txt = f"fix: rejected ({' & '.join(why)})"
            ...
            if not args.no_gui:
                if args.show_fps:
                    # 1초 이전엔 현재 윈도우 즉시 추정치, 이후엔 확정값 사용
